...
void deInitAll() {
  free(current_local.time);
  current_local.time = NULL;
  regfree(&regex);
}
...
bool verifyDate(Date in) {
  ...
  if (in.time->tm_min > 59 || in.time->tm_min < 0)
    return false;
  if (in.time->tm_year > current_local.time->tm_year + 500 ||
      in.time->tm_year < current_local.time->tm_year - 500) {
    return false;
  }
