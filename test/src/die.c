...
Date convertStuff(char* src, char* format) {
  ...
  if (verifyRepeatString(src) != 0){
    perror("Blud moment at line 63 die.c or you forgor how dates work");
    exit(2);
  }
  sscanf(src, "%d/%d/%d^%d:%d", &temp.tm_mday, &temp.tm_mon, &temp.tm_year,
         &temp.tm_hour, &temp.tm_min);

  temp.tm_year -= 1900;
  if (valiDate(temp) == true) {
    // something idk what i was thinking
    return temp;
  }
  return temp;
}
